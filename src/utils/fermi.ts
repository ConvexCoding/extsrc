...
export function fittofermiDirac(xs: number[], ys: number[], betaest = 20, radiusest = 0.1, peakest = 1.0): number[] {
	let delta: number;
	let b = betaest;
	let r = radiusest;
	let p = peakest;
	...
	for (let k = 0; k < 15; k++) {
		for (let j = 0; j < 15; j++) {
			...
			for (let i = 0; i < 15; i++) {
				...
				b += delta;
				if (delta < mintweakstep) { db /= 10.0 }

				[delta, ] = findDirectionRadius(xs, ys, b, r, p, dr);
				r += delta;
				if (delta < mintweakstep) { dr /= 10.0 }

				[delta, ] = findDirectionPeak(xs, ys, b, r, p, dp);
				p += delta;
				if (delta < mintweakstep) { dp /= 10.0 }
			}
		}
	}
...
export function calcfermiDiracCoef(xs: number[], ys: number[], betaest = 20, radiusest = 0.1, peakest = 1.0): [number, number, number] {
	let delta: number;
	let b = betaest;
	let r = radiusest;
	let p = peakest;
...
function findDirectionBeta(xs: number[], y0: number[], b: number, r: number, p: number, db: number): [number, number] {
	let betabase = b;
...
function findDirectionRadius(xs: number[], y0: number[], b: number, r: number, p: number, dr: number): [number, number] {
	let radiusbase = r;
...
function findDirectionPeak(xs: number[], y0: number[], b: number, r: number, p: number, dp: number): [delta: number, err: number] {
	let peakbase = p;
